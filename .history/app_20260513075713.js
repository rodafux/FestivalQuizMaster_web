...
// ÉCOUTE DU SCORE EN ARRIÈRE-PLAN
database.ref('Players/' + playerId).on('value', s => { 
    if (s.val()) {
        actualScore = s.val().Score || 0; 
        
        // Le score s'affiche sur le téléphone SEULEMENT SI on n'est pas en plein milieu d'une question
        // OU SI la régie a déjà révélé la réponse.
    ...
    if (answerRevealed && currentQuestionId) {
        ...
        if (hasAnswered) {
            if (lastAnswerIndex === correctAnswerIndex) {
                ...
                // On lui indique quelle était la bonne option (A, B, C ou D)
...
answerBtns.forEach(b => b.onclick = (e) => {
    ...
    database.ref('Answers').push().set({ 
        ...
        setTimeout(() => { 
            // Si la réponse n'a pas encore été révélée par la régie
            if (!answerRevealed) {
                showScreen('waiting'); 
