...
// Fonction centrale pour vérifier et afficher la question en attente
function checkAndShowQuestion(questionData) {
    if (!questionData || !isRegistrationOpen) return;

    database.ref('Players/' + playerId).once('value').then((playerSnapshot) => {
        if (!playerSnapshot.exists()) return;

        if (questionData.Id !== currentQuestionId) {
    ...
    if (!isRegistrationOpen) {
        ...
        database.ref('Players/' + playerId).once('value').then((playerSnapshot) => {
            if (!playerSnapshot.exists()) {
                ...
                // Si une question est déjà active au moment de l'ouverture/reconnexion
                database.ref('CurrentQuestion').once('value').then((qSnap) => {
                    checkAndShowQuestion(qSnap.val());
                });
            }
        });
    }
...
database.ref('CurrentQuestion').on('value', (snapshot) => {
    checkAndShowQuestion(snapshot.val());
});
...
database.ref('QuizState/TimeRemaining').on('value', (snapshot) => {
    ...
    if (time !== null) {
        ...
        if (time <= 0) answerBtns.forEach(btn => btn.disabled = true);
    }
});
