...
// --- GESTION DU SCORE EN ATTENTE ET DE LA RÉVÉLATION ---
let pendingScore = 0;
...
function syncQuestion(q) {
    ...
    if (q.Id !== currentQuestionId) {
        ...
        // Réinitialise les variables pour la nouvelle question
        lastAnswerIndex = -1;
...
database.ref('CurrentQuestion').on('value', s => {
    if (isLocked && isRegistrationOpen) {
        database.ref('Players/' + playerId).once('value').then(ps => { if (ps.exists()) syncQuestion(s.val()); });
    }
});

// Écoute des changements de score "en arrière-plan"
database.ref('Players/' + playerId).on('value', s => { 
    if (s.val()) {
        pendingScore = s.val().Score || 0; 
        // Met à jour l'écran immédiatement SI la réponse a DÉJÀ été révélée par le Maître
        if (answerRevealed) userScore.innerText = pendingScore;
    }
});
...
// NOUVEAUTÉ : Écoute du signal de la régie "Révéler la Réponse"
database.ref('QuizState/AnswerRevealed').on('value', s => {
    answerRevealed = s.val() || false;
    
    // Seulement si une question est en cours
    if (answerRevealed && currentQuestionId) {
        // MAJ DU SCORE SUR LE TÉLÉPHONE UNIQUEMENT MAINTENANT
        userScore.innerText = pendingScore;
        showScreen('waiting'); 
        
        if (hasAnswered) {
            if (lastAnswerIndex === correctAnswerIndex) {
                waitingText.innerHTML = "<span style='color:#00FF00; font-size:40px;'>✅ BONNE RÉPONSE !</span><br><br>Ton score a été mis à jour.";
            } else {
                waitingText.innerHTML = "<span style='color:#FF4444; font-size:40px;'>❌ MAUVAISE RÉPONSE...</span><br><br>Ton score a été mis à jour.";
            }
        } else {
            waitingText.innerHTML = "<span style='color:#FFAA00; font-size:40px;'>⏳ TEMPS ÉCOULÉ !</span><br><br>Tu n'as pas répondu.";
        }
    }
});
