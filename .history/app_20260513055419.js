...
database.ref('QuizState/RegistrationOpen').on('value', s => {
    ...
    if (!isRegistrationOpen) {
        ...
        waitingText.innerText = "Le quiz n'est pas encore ouvert. Patientez..."; userScore.innerText = "0";
    } else {
        database.ref('Players/' + playerId).once('value').then(ps => {
            if (ps.exists()) {
                loginScreen.classList.add('hidden'); scoreHeader.classList.remove('hidden'); waitingScreen.classList.remove('hidden'); waitingText.innerText = "En attente de la prochaine question...";
                database.ref('CurrentQuestion').once('value', qs => syncQuestion(q.val()));
...
answerBtns.forEach(b => b.onclick = (e) => {
    ...
    database.ref('Answers').push().set({ PlayerId: playerId, QuestionId: currentQuestionId, AnswerIndex: idx, ResponseTimeMs: time }).then(() => {
        setTimeout(() => { questionScreen.classList.add('hidden'); waitingScreen.classList.remove('hidden'); waitingText.innerText = "Réponse envoyée !"; }, 800);
    });
});